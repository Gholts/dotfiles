...
-- =============================================================================
-- == 視窗 (Window) 與 Buffer 管理
-- =============================================================================

-- 關閉當前 buffer
vim.keymap.set('n', '<leader>w', '<cmd>bdelete<cr>', {
    desc = '[W]ipeout current buffer',
    silent = true,
})
...
-- NvimTree - 開關檔案瀏覽器
vim.keymap.set('n', '<D-C-c>', '<cmd>NvimTreeToggle<cr>', {
    desc = 'Toggle [C]ode explorer',
    silent = true,
})

-- 舊的 Netrw 檔案瀏覽器（若主要使用 NvimTree，此項可刪除或註解）
vim.keymap.set('n', '<leader>pv', '<cmd>Ex<cr>', {
    desc = '[P]roject [V]iew (Netrw)',
    silent = true,
})
